...
fn config_followed_immediately_by_another_flag_should_error() {
    // `--config --template foo`: --config is dangling, --template foo parses normally.
...
#[test]
fn template_followed_immediately_by_another_flag_should_error() {
    // `--template --config /x`: --template is dangling, --config /x parses normally.
    let a = parse(&s(&["--template", "--config", "/x"]));
    assert!(a.template_name.is_none());
    assert_eq!(a.config_path, Some(std::path::PathBuf::from("/x")));
    assert!(a.unknown.contains(&"--template".to_string()));
}

#[test]
fn config_followed_by_boolean_flag_should_error() {
    // `--config --debug`: --debug is a no-value flag, so --config is dangling.
    let a = parse(&s(&["--config", "--debug"]));
    assert!(a.config_path.is_none());
    assert!(a.debug);
    assert!(a.unknown.contains(&"--config".to_string()));
}
